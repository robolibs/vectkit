...
namespace geoget {
    ...
    std::string PolygonDrawer::get_polygon_html() {
    ...
    std::string PolygonDrawer::get_single_point_html() {
    ...
    std::string PolygonDrawer::add_point(const std::string &request) {
    ...
    std::string PolygonDrawer::set_point(const std::string &request) {
        size_t body_start = request.find("\r\n\r\n");
        if (body_start == std::string::npos) {
            return "HTTP/1.1 400 Bad Request\r\n\r\nNo body";
        }
    ...
    std::string PolygonDrawer::new_polygon() {
        if (points.size() >= 3) {
    ...
    std::string PolygonDrawer::new_point() {
        if (!points.empty()) {
    ...
    std::string PolygonDrawer::clear_current_only() {
        points.clear();
    ...
    std::string PolygonDrawer::clear_points() {
        points.clear();
        all_polygons.clear();
    ...
    std::string PolygonDrawer::handle_done() {
        if (single_point_mode) {
            if (!points.empty()) {
                all_single_points.push_back(points[0]);
            }
            ...
            for (size_t i = 0; i < all_single_points.size(); ++i) {
                std::cout << "Point " << (i + 1) << ": " << all_single_points[i].lat << ", " << all_single_points[i].lon
                          << std::endl;
            }
            std::cout << "========================\n" << std::endl;
        } else {
    ...
    Point PolygonDrawer::collect_single_point() {
    ...
    void PolygonDrawer::set_datum_from_point(const Point &point) {
        datum.lat = point.lat;
        datum.lon = point.lon;
        datum.alt = 0.0;
    }

    PolygonDrawer::PolygonDrawer(bool select_point)
        : server_fd(-1), is_done(false), single_point_mode(false), select_point(select_point) {}

    PolygonDrawer::PolygonDrawer(const concord::Datum &d)
        : server_fd(-1), is_done(false), single_point_mode(false), datum(d) {}

    PolygonDrawer::~PolygonDrawer() { stop(); }

    bool PolygonDrawer::start(int port) {
        if (server_fd != -1) {
            close(server_fd);
            server_fd = -1;
        }
    ...
    concord::Datum PolygonDrawer::add_datum() {
        single_point_mode = true;
    ...
    concord::Datum PolygonDrawer::get_datum() { return datum; }

    const std::vector<std::vector<Point>> &PolygonDrawer::get_all_polygons() {
        collect_points();
        return all_polygons;
    }

    const std::vector<Point> &PolygonDrawer::get_all_points() {
        collect_single_point();
        return all_single_points;
    }

    std::vector<concord::Polygon> PolygonDrawer::get_polygons() {
        collect_points();
        if (!datum.is_set()) {
            if (select_point) {
    ...
    std::vector<concord::Point> PolygonDrawer::get_points() {
        collect_single_point();
        if (!datum.is_set()) {
            if (select_point) {
                throw std::runtime_error("Datum not set. Call add_datum() first or use constructor with datum.");
